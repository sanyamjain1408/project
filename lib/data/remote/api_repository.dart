...
import '../models/fiat_deposit.dart';
import '../models/future_data.dart';
import 'socket_provider.dart';
import '../../helper/app_helper.dart';
import '../../ui/features/auth/social_login_view.dart';
...
class APIRepository {
  ...
  Map<String, String> authHeader() {
    ...
    mapObj[APIKeyConstants.accept] = APIKeyConstants.vApplicationJson;
    ...
    if (settings?.isEvmWallet == true) {
      mapObj[APIKeyConstants.evmApiSecret] = settings?.evmApiSecret ?? "";
    ...
    /// print("authHeader => $mapObj[APIKeyConstants.token]");
    return mapObj;
  }
  ...
  Future<ServerResponse> registerSocialAccount(SocialLogin loginInfo) async {
    var mapObj = {};
    mapObj[APIKeyConstants.loginType] = loginInfo.type;
    mapObj[APIKeyConstants.userID] = loginInfo.socialId;
    mapObj[APIKeyConstants.email] = loginInfo.email;
    mapObj[APIKeyConstants.name] = loginInfo.name;
    mapObj[APIKeyConstants.accessToken] = loginInfo.token;
    return provider.postRequest(APIURLConstants.socialSignUpLogin, mapObj, authHeader());
  }
  ...
  Future<ServerResponse> walletNetworkAddress(String coinType, String networkType) async {
    var mapObj = {};
  ...
  Future<ServerResponse> withdrawProcess(int coinId, String address, double amount, int networkId, String code, String? memo) async {
    var mapObj = {};
    mapObj[APIKeyConstants.coinId] = coinId;
    mapObj[APIKeyConstants.address] = address;
    mapObj[APIKeyConstants.amount] = amount;
    mapObj[APIKeyConstants.code] = code;
    mapObj[APIKeyConstants.networkId] = networkId;
    mapObj[APIKeyConstants.memo] = memo ?? '';
    return provider.postRequest(APIURLConstants.walletWithdrawalProcess, mapObj, authHeader());
  }

  Future<ServerResponse> walletDepositAddress(int coinId, {String? networkType, int? networkId}) async {
    final mapObj = <String, String>{};
    mapObj[APIKeyConstants.coinId] = "$coinId";
    if(networkType != null) mapObj[APIKeyConstants.networkType] = networkType;
    if(networkId != null) mapObj[APIKeyConstants.networkId] = networkId.toString();
    return provider.postRequest(APIURLConstants.walletDepositAddress, mapObj, authHeader());
  }
  ...
  Future<ServerResponse> swapCoinProcess(int fromCoinId, int toCoinId, double amount) async {
    ...
    mapObj[APIKeyConstants.amount] = amount;
    return provider.postRequest(APIURLConstants.swapCoinApp, mapObj, authHeader(), isDynamic: true);
  }

  Future<ServerResponse> getWalletNetworks(String coin, int type) async {
    final mapObj = <String, String>{};
    mapObj[APIKeyConstants.type] = "$type";
    return provider.getRequest(APIURLConstants.getWalletNetworks + coin, authHeader(), query: mapObj);
  }

  Future<ServerResponse> preWithdrawProcess(String address, Decimal amount, {int? coinId, String? coinType, String? networkType, int? networkId}) async {
    var mapObj = {};
    mapObj[APIKeyConstants.address] = address;
    mapObj[APIKeyConstants.amount] = amount.toDouble();
    if(coinId != null) mapObj[APIKeyConstants.coinId] = coinId;
    if(coinType != null) mapObj[APIKeyConstants.coinType] = coinType;
    if(networkType != null) mapObj[APIKeyConstants.networkType] = networkType;
  ...
  Future<ServerResponse> withdrawalProcess(WithdrawalCreate withdraw) async {
    var mapObj = {};
    mapObj[APIKeyConstants.address] = withdraw.address;
    mapObj[APIKeyConstants.amount] = withdraw.amount;
    mapObj[APIKeyConstants.code] = withdraw.verifyCode;
    mapObj[APIKeyConstants.memo] = withdraw.memo;
    if(withdraw.coinType != null) mapObj[APIKeyConstants.coinType] = withdraw.coinType;
    if(withdraw.coinId != null) mapObj[APIKeyConstants.coinId] = withdraw.coinId;
    if(withdraw.networkType != null) mapObj[APIKeyConstants.networkType] = withdraw.networkType;
    if(withdraw.networkId != null) mapObj[APIKeyConstants.networkId] = withdraw.networkId;
    return provider.postRequest(APIURLConstants.walletWithdrawalProcess, mapObj, authHeader());
  }

  Future<ServerResponse> preWithdrawalProcess(String address, double amount, int coinId, int networkId, String note, String code) async {
    var mapObj = {};
    mapObj[APIKeyConstants.address] = address;
    mapObj[APIKeyConstants.amount] = amount;
    mapObj[APIKeyConstants.coinId] = coinId;
  ...
  Future<ServerResponse> userBankSubmit(BankForm bank) async {
    var mapObj = bank.toJsonSave();
    return provider.postRequest(APIURLConstants.userBankSubmit, mapObj, authHeader());
  }
  ...
  Future<ServerResponse> getGiftCardRedeemCode(String uid, String password) async {
    var mapObj = <String, String>{};
    mapObj[APIKeyConstants.cardUid] = uid;
    mapObj[APIKeyConstants.password] = password;
    return provider.postRequest(APIURLConstants.getGiftCardRedeemCode, mapObj, authHeader());
  }
  ...
  Future<ServerResponse> p2pWalletBalanceTransfer(String coinType, double amount, int type) async {
    ...
    mapObj[APIKeyConstants.amount] = amount;
    mapObj[APIKeyConstants.type] = type;
    return provider.postRequest(APIURLConstants.p2pWalletBalanceTransfer, mapObj, authHeader(), isDynamic: true);
  }
  ...
  Future<ServerResponse> checkCoinTransaction(int netId, int coinId, String transactionId) async {
    final mapObj = {};
    mapObj[APIKeyConstants.networkId] = netId;
    mapObj[APIKeyConstants.coinId] = coinId;
    mapObj[APIKeyConstants.trxId] = transactionId;
    return provider.postRequest(APIURLConstants.checkCoinTransaction, mapObj, authHeader());
  }

  Future<ServerResponse> getLimitOrderTolerance(int baseCoinId, int tradeCoinId) async {
    var mapObj = <String, String>{};
  ...
  Future<ServerResponse> getDashBoardData(String key) async {
    return provider.getRequest(APIURLConstants.getAppDashboard + key, authHeader(), isDynamic: true);
  }
  ...
  Future<ServerResponse> getTradeHistoryList(int baseCoinId, int tradeCoinId, String orderType) async {
    ...
    mapObj[APIKeyConstants.tradeCoinId] = tradeCoinId.toString();
    mapObj[APIKeyConstants.orderType] = orderType;
    return provider.getRequest(APIURLConstants.getMyAllOrdersApp, authHeader(), query: mapObj);
  }

  Future<ServerResponse> getSelfProfile() async {
    return provider.getRequest(APIURLConstants.getProfile, authHeader(), isDynamic: true);
  }
  ...
  Future<ServerResponse> getCommonSettings() async {
    return provider.getRequest(APIURLConstants.getCommonSettingsWithLanding, authHeader(), isDynamic: true);
  }
  ...
  Future<ServerResponse> getWalletTotalValue() async {
  ...
  Future<ServerResponse> getWalletWithdrawal(String coinType) async {
    return provider.getRequest(APIURLConstants.getWalletWithdrawal + coinType, authHeader());
  }
  ...
  Future<ServerResponse> getActivityList(int page, String type, {String? search, bool? isFiat}) async {
    ...
    String url = AppChecker.getHistoryUrl(type, isFiat: isFiat);
    if (url == APIURLConstants.getReferralHistory) {
  ...
  Future<ServerResponse> getFiatWithdrawal() async => provider.getRequest(APIURLConstants.getFiatWithdrawal, authHeader());

  // Future<ServerResponse> getUserBankList() async => provider.getRequest(APIURLConstants.getUserBankList, authHeader());

  Future<ServerResponse> getBankList() async => provider.getRequest(APIURLConstants.getBankList, authHeader());

  Future<ServerResponse> getBankForms() async => provider.getRequest(APIURLConstants.getBankForms, authHeader());

  Future<ServerResponse> userUserBank(int id) async => provider.getRequest("${APIURLConstants.userUserBank}$id",  authHeader());

  Future<ServerResponse> userBankDelete(int id) async => provider.getRequest("${APIURLConstants.userBankDelete}$id",  authHeader());

  Future<ServerResponse> getCurrencyList() async => provider.getRequest(APIURLConstants.getCurrencyList, authHeader());
  ...
  Future<ServerResponse> getCoinsByNetwork(int netId) async {
    final mapObj = <String, String>{};
    mapObj[APIKeyConstants.networkId] = "$netId";
    return provider.getRequest(APIURLConstants.getCoinNetwork, authHeader(), query: mapObj);
  }
  ...
  Future<ServerResponse> getGiftCardCheck(String code) async {
    var mapObj = <String, String>{};
    mapObj[APIKeyConstants.code] = code;
    return provider.getRequest(APIURLConstants.getGiftCardCheck, authHeader(), query: mapObj, isDynamic: true);
  }
  ...
  Future<ServerResponse> getCoinList({bool? isDeposit, bool? isWithdraw, int? currencyType, bool? checkDeposit}) async {
    var mapObj = <String, String>{};
    if(isDeposit != null) mapObj[APIKeyConstants.isDeposit] = isDeposit.toString();
    if(isWithdraw != null) mapObj[APIKeyConstants.isWithdrawal] = isWithdraw.toString();
    if(currencyType != null) mapObj[APIKeyConstants.currencyType] = currencyType.toString();
    if(checkDeposit == true) mapObj[APIKeyConstants.checkDeposit] = checkDeposit.toString();
    return provider.getRequest(APIURLConstants.getCoinList, authHeader(), query: mapObj);
  }
