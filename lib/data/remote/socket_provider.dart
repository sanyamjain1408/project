...
class SocketProvider extends GetxController {
  ...
  void parseMessage(SocketResponse socketResponse) {
    printFunction("webSocket channel:event", "${socketResponse.channel} : ${socketResponse.event}");
    // printFunction("webSocket data", "${socketResponse.data}");
    ...
    switch (socketResponse.event) {
      ...
      case SocketConstants.eventMarketOverviewTopCoinList:
        ...
        return;
      case SocketConstants.eventMarketCoinPair:
        final data = json.decode(socketResponse.data ?? "");
        _callListeners(socketResponse.channel ?? "", SocketConstants.eventMarketCoinPair, data);
        return;
    ...
    if (socketResponse.event == "${SocketConstants.eventOrderPlace}-${gUserRx.value.id}") {
      ...
      return;
    }else if (socketResponse.event == "${SocketConstants.eventProcess}-${gUserRx.value.id}") {
      final sData = socketUserHistoryFromJson(socketResponse.data ?? "");
      _callListeners(socketResponse.channel ?? "", socketResponse.event ?? "", sData);
      return;
    }
