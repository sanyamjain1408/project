...
class SwapController extends GetxController {
  ...
  Future<void> getCoinSwapApp({Wallet? preWallet, CoinPair? pair}) async {
    APIRepository().getCoinSwapApp().then((resp) {
      if (resp.success) {
        ...
        setInitialSelections(preWallet: preWallet, pair: pair);
        getAndSetCoinRate();
      } else {
  ...
  void setInitialSelections({Wallet? preWallet, CoinPair? pair}) {
    if (preWallet != null) {
      ...
      final wallet = walletList.firstWhereOrNull((element) => element.coinType == pair.parentCoinName);
      if (wallet != null) selectedFromCoin.value = wallet;
    ...
    if (walletList.length > 1) {
      if (pair != null) {
        final wallet = walletList.firstWhereOrNull((element) => element.coinType == pair.childCoinName);
        if (wallet != null) selectedToCoin.value = wallet;
      } else if (walletList[1].coinType == selectedFromCoin.value.coinType) {
      ...
      if (resp.success) {
        ...
        if (success) {
          ...
          if (Get.isRegistered<WalletController>()) {
            Future.delayed(const Duration(seconds: 2), () => Get.find<WalletController>().getWalletList(WalletViewType.spot, () {}));
          }
        }
      }
    }, onError: (err) {
