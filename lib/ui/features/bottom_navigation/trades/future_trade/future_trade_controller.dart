  ...
  void getFutureTradeData() {
    ...
    APIRepository().getFutureTradeAppDashboard(selectedCoinPair.value.getCoinPairKey()).then((resp) {
      ...
      if (resp.success) {
        ...
        setSelectedPrice.value = futureDashboardData.value.orderData?.total?.tradeWallet?.lastPrice;
        fBalance.value = futureDashboardData.value.orderData?.total?.baseWallet?.balance ?? 0;
        ...
        FavoriteHelper.checkFavorite(selectedCoinPair.value, FromKey.future, (pair) => selectedCoinPair.value = pair);
