  ...
  void getGiftCardCheck(String code, Function(GiftCard) onSuccess) {
    ...
    APIRepository().getGiftCardCheck(code).then((resp) {
      ...
      if (resp.success) {
        ...
        if (success) {
          final card = GiftCard.fromJson(resp.data[APIKeyConstants.data][APIKeyConstants.card]);
          final banner = GiftCardBanner.fromJson(resp.data[APIKeyConstants.data][APIKeyConstants.banner]);
          card.banner = banner;
          onSuccess(card);
        }else {
          showToast(message);
