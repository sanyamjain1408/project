...
class EarnProduct {
  ...
  final String? coinIcon;    // ← backend URL, same as LandingMarketView
  ...
  factory EarnProduct.fromJson(Map<String, dynamic> json) {
    return EarnProduct(
      ...
      coinIcon:  json['coin_icon'] ?? json['icon'] ?? json['image'],   // ← try multiple keys
    );
  }
}
...
class EarnSubscription {
  ...
  final String? coinIcon;    // ← for display
  ...
  Future<bool> subscribe({
    ...
    try {
      final res  = await http.post(
        ...
        body: jsonEncode({'product_id': productId, 'amount': amount, 'auto_reinvest': autoReinvest}),
      );
      hideLoadingDialog();
      final json    = jsonDecode(res.body);
      final success = json['success'] == true;
      showToast(json['message'] ?? (success ? 'Subscribed!' : 'Failed'), isError: !success);
      if (success) { await fetchPositions(); await fetchBalances(); }
      return success;
    } catch (e) {
      hideLoadingDialog();
      showToast(e.toString());
