...
class _KYCScreenState extends State<KYCScreen> {
                ...
                children: [
                  if (kycSettingsRx.value.enabledKycType == 1)
                    ...
                    if (kycSettingsRx.value.enabledKycUserDetails?.persona?.isVerified == 1)
                      Column(children: [
                        vSpacer30(),
                        Icon(Icons.how_to_reg, color: context.theme.focusColor, size: Dimens.iconSizeLargeExtra),
                        vSpacer10(),
                        TextRobotoAutoBold("Kyc Verified Successfully".tr),
                      ])
                    else
                      Column(children: [
                        vSpacer30(),
                        ...
                        vSpacer10(),
                        TextRobotoAutoBold("Verify your identity".tr),
                        vSpacer20(),
                        buttonRoundedMain(text: "Start".tr, width: context.width / 2, onPress: () => _startPersonaVerification())
                      ])
                  else
                    Column(children: [
                      vSpacer30(),
                      ...
                      vSpacer10(),
                      TextRobotoAutoBold("Kyc disabled".tr),
                    ])
                ],
              ),
      ),
  ...
  Column _manualVerificationView(KycDetails details) {
    return Column(children: [
      Align(alignment: Alignment.centerLeft, child: TextRobotoAutoNormal("KYC Verification List".tr, fontSize: Dimens.fontSizeMidExtra)),
  ...
  Widget _kycItemView(KycObject? kyc, String title, String imagePath) {
    final sData = AppChecker.getIdVerificationStatusData(kyc?.status);
    return Column(
      children: [
        InkWell(
                  ...
                  child: Stack(
                    children: [
                      ...
                      Container(
                          ...
                          height: Dimens.iconSizeMid,
                          width: context.width / 2.5,
                          child: TextRobotoAutoBold(sData.first, color: Colors.white))
                    ],
                  ),
                ),
        ...
        TextRobotoAutoNormal(title, fontSize: Dimens.fontSizeMidExtra, textAlign: TextAlign.center)
      ],
    );
  }
      ...
      child: ListView(
        ...
        children: [
          ...
          if (kyc?.status == IdVerificationStatus.notSubmitted || kyc?.status == IdVerificationStatus.rejected)
            buttonRoundedMain(text: "Upload".tr, onPress: () => _checkInputData(kyc)),
          vSpacer10(),
        ],
      ),
    );
  ...
  Widget _showUploadImage(File file, PhotoType photoType, String? prePath) {
    ...
    return InkWell(
      child: Container(
        ...
        width: context.width,
        margin: const EdgeInsets.all(Dimens.paddingLarge),
        decoration: boxDecorationRoundCorner(color: context.theme.secondaryHeaderColor),
